...
const SECTIONS = [
    {
        ...
        description: "I build reactive, accessible, and performant user interfaces that feel instantaneous.",
    ...
    },
    {
        ...
        description: "Designing robust APIs and data schemas that handle millions of requests without breaking a sweat.",
    ...
    },
    {
        ...
        description: "Orchestrating containerized environments and serverless functions for maximum uptime.",
...
const FloatingIcons = ({ skills, parentRef }: { skills: any[], parentRef: React.RefObject<HTMLElement> }) => {
...
const StackCard = ({ data, index }: { data: typeof SECTIONS[0], index: number }) => {
    ...
    return (
        ...
        >
            ...
            {/* Glowing Orb */}
            <div className={`glow-orb absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-gradient-to-r ${data.color} rounded-full blur-[120px] opacity-20 pointer-events-none`}></div>
            ...
            <div className="relative z-10 w-full max-w-5xl px-6 md:px-12 grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
                ...
                <div>
                    <div className="anim-text flex items-center gap-4 mb-6">
                        <span className="text-xs font-bold uppercase tracking-[0.2em] text-white/40">0{index + 1} — {data.id}</span>
 ...
 <nav className="fixed top-0 left-0 w-full z-[100] flex justify-between items-center px-8 md:px-12 py-6 mix-blend-difference text-white">
          ...
          <button className="group flex items-center gap-3 text-sm font-inter uppercase tracking-widest hover:text-cyan-400 transition-colors">
            ...
            <div className="h-[50vh] flex items-center justify-center bg-white text-black relative z-20">
                <div className="text-center">
                    <h2 className="text-4xl font-bold tracking-tight mb-4">Ready to collaborate?</h2>
